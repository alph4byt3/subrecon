...
for domain in ${list[@]}; do
  ...
  shuffledns -mode bruteforce -d $domain -w /usr/share/seclists/Discovery/DNS/best-dns-wordlist.txt -r ~/Tools/resolvers.txt -o bruteforced-subdomains.txt -silent > /dev/null
  ...
  amass enum -d $domain -silent > /dev/null
  
  amass subs -names -d $domain > amass-subdomains.txt
